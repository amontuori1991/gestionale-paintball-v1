...
namespace Full_Metal_Paintball_Carmagnola.Models
{
    public class TesseramentoViewModel : IValidatableObject
    {
        ...
        public string? TipoDocumentoEstero { get; set; }

        [Display(Name = "Numero documento")]
        public string? NumeroDocumentoEstero { get; set; }
        ...
        [Required(ErrorMessage = "Seleziona se vuoi ricevere comunicazioni newsletter")]
        [Display(Name = "Vuoi ricevere comunicazioni e newsletter?")]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (NatoEstero)
            {
                ...
                if (string.IsNullOrWhiteSpace(ComuneResidenza))
                    yield return new ValidationResult("Il comune o la città di residenza è obbligatorio.", new[] { nameof(ComuneResidenza) });

                if (string.IsNullOrWhiteSpace(TipoDocumentoEstero))
                    yield return new ValidationResult("Il tipo di documento è obbligatorio per i nati all'estero.", new[] { nameof(TipoDocumentoEstero) });

                if (string.IsNullOrWhiteSpace(NumeroDocumentoEstero))
                    yield return new ValidationResult("Il numero del documento è obbligatorio per i nati all'estero.", new[] { nameof(NumeroDocumentoEstero) });
            }
            else
            {
        ...
        public Tesseramento ToEntity(string firmaPath)
        {
            return new Tesseramento
            {
                ...
                TipoDocumentoEstero = this.NatoEstero ? this.TipoDocumentoEstero : null,
                NumeroDocumentoEstero = this.NatoEstero ? this.NumeroDocumentoEstero : null,
                ComuneResidenza = this.ComuneResidenza ?? this.NazioneResidenza ?? string.Empty,
                Email = this.Email,
                ...
                NoTesseramento = this.NoTesseramento,
                NewsletterConsent = this.NewsletterConsent == true,
