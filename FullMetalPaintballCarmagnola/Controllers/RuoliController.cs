...
public class RuoliController : Controller
{
    ...
    public async Task<IActionResult> GestionePermessi()
    {
        ...
        ViewBag.RegisterEnabled = await IsSettingEnabledAsync(RegisterEnabledSettingKey);
        ViewBag.TesseramentoLiberoEnabled = await IsSettingEnabledAsync(TesseramentoLiberoEnabledSettingKey);
    ...
    {
        var enabled = registerEnabled == "on";
        await SaveSettingAsync(RegisterEnabledSettingKey, enabled);
    ...
    [HttpPost]
    public async Task<IActionResult> AggiornaTesseramentoLibero(string tesseramentoLiberoEnabled)
    {
        var enabled = tesseramentoLiberoEnabled == "on";
        await SaveSettingAsync(TesseramentoLiberoEnabledSettingKey, enabled);

        TempData["SuccessMessage"] = enabled
            ? "Tesseramento pubblico senza partita abilitato."
            : "Tesseramento pubblico senza partita disabilitato.";
    ...
    private async Task SaveSettingAsync(string key, bool enabled)
    {
    ...
    private async Task<bool> IsSettingEnabledAsync(string key)
    {
