...
namespace Full_Metal_Paintball_Carmagnola.Services
{
    ...
    public class EmailSender : IEmailService
    {
        ...
        public async Task SendEmailAsync(string email, string subject, string htmlMessage)
        {
            ...
            var smtpConfig = BuildSmtpConfig(smtpSettings);
...
<body>
  <div class='container'>
    ...
    <table class='info-table'>
      ...
      {(model.NatoEstero ? $"<tr><th>Nazione di Nascita:</th><td>{model.NazioneNascita}</td></tr><tr><th>Città di Nascita:</th><td>{model.CittaNascita}</td></tr><tr><th>Cittadinanza:</th><td>{model.NazioneCittadinanza}</td></tr><tr><th>Nazione Residenza:</th><td>{model.NazioneResidenza}</td></tr><tr><th>Documento:</th><td>{model.TipoDocumentoEstero} {model.NumeroDocumentoEstero}</td></tr>" : $"<tr><th>Comune di Nascita:</th><td>{model.ComuneNascita}</td></tr>")}
    ...
    <div class='footer'>
      ...
      <p>Questo messaggio è generato automaticamente, ti preghiamo di non rispondere.</p>
        ...
        private static SmtpConfig BuildSmtpConfig(IConfigurationSection smtpSettings)
        {
            var host = smtpSettings["Host"];
            var portValue = smtpSettings["Port"];
            var enableSslValue = smtpSettings["EnableSsl"];
            ...
            {
                throw new InvalidOperationException("Configurazione SMTP incompleta. Imposta SmtpSettings__Host, SmtpSettings__Port, SmtpSettings__EnableSsl, SmtpSettings__User e SmtpSettings__Password nei secret/variabili d'ambiente.");
            }

            return new SmtpConfig(
                host,
                int.Parse(portValue),
                bool.Parse(enableSslValue),
                user,
                password);
        }

        private sealed record SmtpConfig(string Host, int Port, bool EnableSsl, string User, string Password);
    }
}
